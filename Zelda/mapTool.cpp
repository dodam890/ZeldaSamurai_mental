...
void mapTool::setupTiles(void)
{
	...
	for (int i = 0; i < SAMPLETILEY4; i++)
	{
		for (int j = 0; j < SAMPLETILEX4; j++)
		{
			_sampleTiles4[i*SAMPLETILEX4 + j].terrainFrameX = j;
			_sampleTiles4[i*SAMPLETILEX4 + j].terrainFrameY = i;
			_sampleTiles4[i*SAMPLETILEX4 + j].objFrameX = j;
			_sampleTiles4[i*SAMPLETILEX4 + j].objFrameY = i;
			_sampleTiles4[i*SAMPLETILEX4 + j].imgKind = 1;
			_sampleTiles4[i*SAMPLETILEX4 + j].rcTile = RectMake(WINSIZEX + (j * SAMPLETILESIZE), (i * SAMPLETILESIZE) + 200, SAMPLETILESIZE, SAMPLETILESIZE);
		}
	}
...
void mapTool::nowDrawInterFaceKind()
{
	...
	case SET_TERRAIN_RC:
		...
		if (_sampleTiles4[0].rcTile.left < WINSIZEX)
		{
			for (int i = 0; i < SAMPLETOTAL4; i++)
			{
				_sampleTiles4[i].rcTile.left += 10;
				_sampleTiles4[i].rcTile.right += 10;
			}
		}
		break;
	...
	case SET_ENEMY_RC:
		...
		if (_sampleTiles4[0].rcTile.left > WINSIZEX - _imgTile8->getWidth() - 25)
		{
			for (int i = 0; i < SAMPLETOTAL4; i++)
			{
				_sampleTiles4[i].rcTile.left -= 10;
...
void mapTool::drawSampleBack()
{
	...
	case SET_ENEMY_RC:
		_imgSample4->render(getMemDC(), _sampleTiles4[0].rcTile.left - 5, _sampleTiles4[0].rcTile.top - 5);
		break;
...
void mapTool::drawSampleTile()
{
	...
	_imgTile2->render(getMemDC(), _sampleTiles2[0].rcTile.left, _sampleTiles2[0].rcTile.top);
	_imgTile5->render(getMemDC(), _sampleTiles3[0].rcTile.left, _sampleTiles3[0].rcTile.top);
	_imgTile7->render(getMemDC(), _sampleTiles4[0].rcTile.left, _sampleTiles4[0].rcTile.top);
}
...
void mapTool::drawNowInterfaceRect()
{
	...
	case SET_OBJECT2_RC:
		...
		break;
	case SET_ENEMY_RC:
		TextOut(getMemDC(), WINSIZEX - 200, 700, "적입니다", strlen("적입니다"));
...
void mapTool::loadImage()
{
	...
	_imgTile7 = IMAGEMANAGER->addFrameImage("mapTiles7", "image/sample_enemy_30.bmp", 0, 0, 90, 30, SAMPLETILEX4, SAMPLETILEY4, true, RGB(255, 0, 255));
	_imgTile8 = IMAGEMANAGER->addFrameImage("mapTiles8", "image/sample_enemy_80.bmp", 0, 0, 240, 80, SAMPLETILEX4, SAMPLETILEY4, true, RGB(255, 0, 255));

	_imgSample1 = IMAGEMANAGER->addImage("sample1", "image/MAPTOOL_SAMPLE_TER_1.bmp", 190, 130, true, RGB(255, 0, 255));
	_imgSample2 = IMAGEMANAGER->addImage("sample2", "image/MAPTOOL_SAMPLE_OBJ_1.bmp", 430, 310, true, RGB(255, 0, 255));
	_imgSample3 = IMAGEMANAGER->addImage("sample3", "image/MAPTOOL_SAMPLE_OBJ_2.bmp", 460, 340, true, RGB(255, 0, 255));
	_imgSample4 = IMAGEMANAGER->addImage("sample4", "image/MAPTOOL_SAMPLE_OBJ_3.bmp", 280, 100, true, RGB(255, 0, 255));
...
void mapTool::selectTile()
{
	...
	for (int i = 0; i < SAMPLETILEX4 * SAMPLETILEY4; i++)
	{
		if (PtInRect(&_sampleTiles4[i].rcTile, _ptMouse))
		{
			_currentTile.objFrameX = _sampleTiles4[i].objFrameX;
			_currentTile.objFrameY = _sampleTiles4[i].objFrameY;
			break;
		}
	}
...
void mapTool::drawTileName()
{
	...
	for (int i = 0; i < TILETOTAL; i++)
	{
		...
		case OBJ_SNAIL:
			TextOut(getMemDC(), _tiles[i].rc.left + 10, _tiles[i].rc.top + 80, "달팽이", strlen("달팽이"));
			break;

		case OBJ_OCTOPUS:
			TextOut(getMemDC(), _tiles[i].rc.left + 10, _tiles[i].rc.top + 80, "문어", strlen("문어"));
			break;
		case OBJ_SLIME:
			TextOut(getMemDC(), _tiles[i].rc.left + 10, _tiles[i].rc.top + 80, "슬라임", strlen("슬라임"));
			break;

		default:
			TextOut(getMemDC(), _tiles[i].rc.left + 10, _tiles[i].rc.top + 80, "?", strlen("?"));
			break;
		}
...
OBJECT mapTool::objSelect3(int frameX, int frameY)
{
